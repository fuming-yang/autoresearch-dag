...
This is a DAG, not a tree. `expand` asks for one logical change from one finished parent. `merge` supplies two finished parents, their hypotheses, results, and direct Git diff. The agent decides whether their ideas are distinct and compatible and, if so, semantically synthesizes one coherent `train.py` from the better-result parent. It does not invoke `git merge` or search for a common ancestor. Redundant or conflicting proposals are rejected. Existing expand/merge frequency, multi-parent commits, Git worktrees, DAG metadata, result parsing, validation, evaluation, and failure/retry behavior remain in effect; there is no new merge gating.
...
Expand samples one parent from this distribution. Merge retains the existing operation probability and untried-pair rule, but samples two distinct parents without replacement from the same credit distribution. Each actual parent use is counted (`T += 1`, `n_v += 1`); a merge adds two uses. Node metadata records the operation, selected parent IDs, losses, Q/E/H/C, probability, pre-draw `n_v`, and current `T`.

## Candidate agent contract

The inner candidate-generation command explicitly uses `codex exec --model gpt-5.6-luna`; it must never depend on a default or fallback model. The agent writes an untracked `decision.json` with `status` and `hypothesis`, does not commit, and does not run the full training experiment. The orchestrator alone creates commits, launches training, parses results, updates the DAG, and releases the GPU. Failed runs remain non-eligible.
