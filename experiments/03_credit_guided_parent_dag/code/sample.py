...
def choose(state, seed=None):
    """Choose the operation, then sample parent(s) by credit softmax."""
    ...
    if mode_draw >= 1.0 - MERGE_PROBABILITY and untried_pairs:
        # Ordered weights are sequential softmax sampling without replacement,
        # conditioned on the existing constraint that a merge pair is untried.
        ordered_pairs, pair_weights = [], []
        for left, right in untried_pairs:
            for first, second in ((left, right), (right, left)):
                ...
                remaining_weights = [math.exp((record["credit"] - remaining_max) /
                                              credit_config(state)["temperature"])
                ...
                ordered_pairs.append((first, second, first_p, second_p))
                pair_weights.append(first_p * second_p)
        first, second, first_p, second_p = _weighted_choice(ordered_pairs, pair_weights, rng)
        return {"mode": "merge", "parents": [first, second], "score": mode_draw,
                "reason": "credit-softmax merge sampled without replacement from an untried pair",
                "parent_selection": {"operation": "merge", "current_T": by_id[first]["current_T"],
                    "selected": [_selection_snapshot(by_id[first], first_p),
                                 _selection_snapshot(by_id[second], second_p)]}}
    probabilities = [record["selection_probability"] for record in credits]
    parent = _weighted_choice(credits, probabilities, rng)
    reason = ("credit-softmax expand selection" if mode_draw < 1.0 - MERGE_PROBABILITY
              else "merge had no untried pair; fell back to credit-softmax expand selection")
